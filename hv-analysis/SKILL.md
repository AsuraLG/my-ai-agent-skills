...
1. **确认虚拟环境可用**：本skill依赖 `uv` 提供的虚拟环境，如果在[skill目录]中未找到虚拟环境目录 `.venv`，则先使用 `uv venv --seed` 命令创建。
2. **确认PDF转换脚本可用**：本Skill自带 `scripts/md_to_pdf.py`（基于WeasyPrint），用于将最终Markdown报告转为排版精美的PDF。该脚本**必须在虚拟环境中使用，如果当前环境没有激活虚拟环境，则先激活虚拟环境**，然后在虚拟环境中确保依赖已安装：`pip3 install weasyprint markdown --break-system-packages`。
3. **写作风格**：本Skill已内置完整的写作风格指南（见下文"写作风格"部分），无需额外加载其他skill。
4. **创建研究素材工作目录**：每次研究必须创建一个本地临时目录来存放中间研究素材。这是为了**避免大量原始素材留在对话上下文中导致响应变慢甚至无法继续**。目录结构：
   ```
...
子Agent在默认权限模式下可能无法使用WebSearch/WebFetch等联网工具。如果子Agent报告权限被拒绝，改为以下策略：
...
报告写完后，在**虚拟环境**中使用本Skill自带的 `scripts/md_to_pdf.py` 脚本将Markdown转为排版精美的PDF。

### 转换流程

1. **先完成Markdown稿件**：将完整报告写为标准Markdown格式，保存为 `[研究对象名称]_横纵分析报告.md`
2. **安装依赖**（如未安装）：`pip3 install weasyprint markdown --break-system-packages`
3. **运行转换脚本（必须在虚拟环境中执行）**：[input.md]代表步骤1中完成的Markdown文件的真实路径，[output.pdf]代表转换后的PDF文件的真实路径，[研究对象名称]代表用户在执行skill时要研究的对象名称
   ```bash
   python3 [skill目录]/scripts/md_to_pdf.py [input.md] [output.pdf] --title "[研究对象名称]" --author "Asura"
   ```
