...
1. **确认Python环境可用**：本skill依赖 `uv` 提供的虚拟环境，如果在[skill目录]中未找到虚拟环境目录 `.venv`，则在[skill目录]下使用 `uv sync` 命令创建虚拟环境并安装相关依赖。
2. **写作风格**：本Skill已内置完整的写作风格指南（见下文"写作风格"部分），无需额外加载其他skill。
3. **创建研究素材工作目录**：每次研究必须创建一个本地临时目录来存放中间研究素材。这是为了**避免大量原始素材留在对话上下文中导致响应变慢甚至无法继续**。目录结构：
   ```
   /tmp/hv-research-[研究对象名称]/
   ├── 01_longitudinal.md  # 纵向信息素材
   ├── 02_horizontal.md    # 横向信息素材
   ├── 03_supplementary.md # 补充信息素材（如有）
   └── merged_notes.md     # 合并整理后的研究笔记
   ```
   在环境准备阶段就用 `mkdir -p` 创建好这个目录。
...
研究素材体量很大，如果全部留在对话上下文中，会导致后续响应越来越慢甚至无法继续。因此必须遵循以下原则：

1. **子Agent必须将结果写入本地文件**，而不是在返回消息中包含完整内容。每个子Agent的prompt中必须明确要求：将所有收集到的信息写入指定的本地文件路径（前置准备阶段创建的工作目录中的对应文件）。
2. **子Agent返回的消息应该只是简短摘要**（200字以内），说明收集了哪些方面的信息、信息是否充分、有哪些缺口需要补充。详细内容全部在文件中。
3. **主Agent不要在对话中展开或复述素材内容**。后续写报告时，直接用Read工具从文件读取素材，按章节分段写入报告文件。
...
**子Agent prompt模板**（每个子Agent的prompt中必须包含以下内容）：

> **输出要求（最重要）**：
> - 将你收集到的所有信息写入文件：`[对应的本地文件路径]`
> - 文件格式为Markdown，按主题分节组织，每条关键信息标注来源URL
> - 完成后返回一段200字以内的简短摘要，说明收集了哪些信息、信息质量如何、有哪些明显缺口
> - **不要在返回消息中包含完整的研究内容**，所有详细内容都写入文件
>
> **联网工具使用指南**：
> - **web-access**：如果用户环境中安装了 `web-access` skill，优先加载它并遵循其指引，它提供更强的浏览器CDP能力
> - **WebSearch**：用于搜索发现信息来源，获取摘要和关键词结果
> - **WebFetch**：当已知具体URL时，用于从页面定向提取内容
> - 搜索策略：如果用户环境中安装了 `web-access` skill，优先使用它，否则用WebSearch发现信息来源和线索，找到具体URL后用WebFetch深入提取
...
子Agent在默认权限模式下可能无法使用WebSearch/WebFetch等联网工具。子Agent在联网信息收集之前必须先确认是否可以使用WebSearch/WebFetch等联网工具。如果子Agent报告权限被拒绝，改为以下策略：
- **在主Agent中直接使用WebSearch和WebFetch进行搜索**，但搜索结果不要留在对话中展开——立刻将结果追加写入工作目录中的对应素材文件
- 或者使用 `mode: auto` 启动子Agent以绕过权限限制

子Agent在默认权限模式下可能无法访问目录 `/tmp/hv-research-[研究对象名称]` 并写入。子Agent在联网信息收集之前必须先确认自身是否可以访问该目录以及是否可以写入，如果不行，则尝试使用bash命令等方式绕开限制，如果都失败了，则告知用户，并改为在 [工作目录路径] 下创建目录 `hv-research-[研究对象名称]` ，在后续流程中使用该目录

### 信息收集完成后的合并步骤

所有子Agent完成后（或主Agent直接搜索完成后），执行以下合并步骤：

1. 用Read工具**快速扫描**各素材文件，检查信息充分性（不要在对话中展开文件全文）
2. 如果发现明显信息缺口，进行针对性补充搜索，结果追加到对应文件中
3. 确认信息充分后，告知用户"信息收集完成，素材已保存在 [工作目录路径]，开始撰写报告"
...
搜索完成后，通过Read工具**快速浏览**各素材文件的标题和结构（不要在对话中复述内容），检查：
- 纵向：能讲出一个完整的故事吗？有没有明显的信息断层？
- 横向：竞品列表完整吗？有没有遗漏主要玩家？每个竞品的信息够做对比吗？
- 来源：关键事实有可靠来源支撑吗？有没有只靠单一来源就下判断的？

信息不够就再补搜，补充的内容追加到对应的素材文件中。不要凑合。

---

## 报告撰写阶段的上下文管理

从第二步开始进入报告撰写阶段。撰写时遵循以下原则避免上下文膨胀：

1. **按章节分段写入**：不要试图一次性生成整篇报告。按章节（纵向分析、横向分析、交汇洞察等）分别撰写，每个章节写完立刻追加到报告Markdown文件中。
2. **从文件读取素材**：撰写每个章节前，用Read工具从工作目录中的素材文件读取对应部分的信息。读取后直接用于写作，不要在对话中复述或总结素材。
3. **写完即落盘**：每个章节写完后立刻用Write/Edit工具写入报告文件，不要在对话中积累多个章节后再一起写入。
4. **定期向用户汇报进度**：每完成一个主要章节，简短告知用户进度（如"纵向分析已完成，约8000字，开始写横向分析"）。
...
报告写完后，使用本Skill自带的 `scripts/md_to_pdf.py` 脚本将Markdown转为排版精美的PDF，使用方法见转换流程。

### 转换流程

1. **先完成Markdown稿件**：将完整报告写为标准Markdown格式，保存为 `[研究对象名称]_横纵分析报告.md`
2. **运行转换脚本**：[input.md]代表步骤1中完成的Markdown文件的真实路径，[output.pdf]代表转换后的PDF文件的真实路径，[研究对象名称]代表用户在执行skill时要研究的对象名称
   ```bash
   uv run --project [skill目录] python [skill目录]/scripts/md_to_pdf.py [input.md] [output.pdf] --title "[研究对象名称]" --author "Asura"
   ```
...
- 紧接标题后可用 `> 研究时间：... | 所属领域：... | 研究对象类型：...` 格式写元信息行，会被提取到封面。研究时间使用YYYY-MM-DD格式。
...
- Markdown文件命名为 `[研究对象名称]_横纵分析报告.md`，如果用户没有特别指定，默认保存到工作目录中。
- PDF文件命名为 `[研究对象名称]_横纵分析报告.pdf`，如果用户没有特别指定，默认保存到工作目录中。
