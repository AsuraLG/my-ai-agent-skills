...
1. **确认虚拟环境可用**：本skill依赖 `uv` 提供的虚拟环境，如果在[skill目录]中未找到虚拟环境目录 `.venv`，则先使用 `uv venv` 命令创建。
2. **确认PDF转换脚本可用**：本Skill自带 `scripts/md_to_pdf.py`（基于WeasyPrint），用于将最终Markdown报告转为排版精美的PDF。该脚本**必须在虚拟环境中使用，如果当前环境没有激活虚拟环境，则先激活虚拟环境**，然后在虚拟环境中确保存在 `pip3` 或者 `pip` ，并使用 `pip3` 或者 `pip` 校验依赖已安装：`pip3 install weasyprint markdown --break-system-packages` 或者 `pip install weasyprint markdown --break-system-packages`。
...
- 或者尝试使用 `mode: auto` 启动子Agent以绕过权限限制（如果支持的话）
...
2. **安装依赖**（如未安装）：`pip3 install weasyprint markdown --break-system-packages` 或 `pip install weasyprint markdown --break-system-packages`
3. **运行转换脚本（必须在虚拟环境中执行）**：[input.md]代表步骤1中完成的Markdown文件的真实路径，[output.pdf]代表转换后的PDF文件的真实路径，[研究对象名称]代表用户在执行skill时要研究的对象名称
   ```bash
   python [skill目录]/scripts/md_to_pdf.py [input.md] [output.pdf] --title "[研究对象名称]" --author "Asura"
   ```
