...
def parse_mcp_servers(input_str: str) -> List[str]:
    ...
    从用户输入中解析MCP服务器名称
    支持多种格式：
    - 逗号分隔：filesystem,sequential-thinking,memory
    - 空格分隔：filesystem sequential-thinking memory
    - 中文逗号分隔：filesystem，sequential-thinking，memory
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("例如: python disable_mcp_servers.py filesystem sequential-thinking memory")
        sys.exit(1)
